            ...
            <.div(^.className := "list-group",
              <.div(^.className := "list-group-item",
                <.p(^.className := "list-group-item-text",
                  "By default Boot2Docker uses https connections and Certificate based authentication, so to allow Docker UI to connect to Docker Remote API we need first to install the credentials used by Boot2Docker",
                  <.ul(
                    <.li("First, you need to make Chrome trust in the autogenerated CA. Execute this command to add the new CA to your Certificate Trust Settings in your Keychain:", <.br(),
                      <.code("security add-trusted-cert -k ~/Library/Keychains/login.keychain  ~/.boot2docker/certs/boot2docker-vm/ca.pem")
                    ),
                    <.li("Also, you need to add the certificate to your Keychain:", <.br(),
